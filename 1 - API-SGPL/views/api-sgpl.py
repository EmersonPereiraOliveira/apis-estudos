...
class UsuariosResource(Resource):
    def get(self, usuario_id):
        ...
        return {usuario_id: usuario.nome} #Tem q serializar o setor e enviá-lo por completo
        #return [{usuario_id: usuario.nome} for usuario in db.session.query(Usuario).filter_by(id=usuario_id).all() ]#Tem q serializar o setor e enviá-lo por completo
    ...
    def delete(self, usuario_id):
        try:
            ...
            if usuario:
                ...
                db.session.commit()
            return jsonify({'status': 200, 'mensagem': 'Usuario excluído com sucesso'})
...
class UsuariosListResource(Resource):
    ...
    def post(self):
        ...
        if not 'nome' in args or not 'email' in args or not 'senha' in args or not 'setor' in args or not 'tipo' in args:
            # Retornando erro
            return {'message': 'Parâmetros obrigatórios ausentes.'}, 400
